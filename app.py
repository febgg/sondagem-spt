...
def extrair_spt_da_linha(texto_linha, metro_alvo):
    ...
    if numeros:
        # Geralmente o SPT é o último número da linha ou um valor coerente (ex: entre 0 e 70)
        for num in reversed(numeros):
            valor = int(num)
            if 0 <= valor <= 70: # Filtro para evitar pegar anos ou números de furos
                return valor
    return "-"
...
def processar_sondagem_sequencial(pdf_file):
    ...
    dados_finais = []
    max_metro = max(banco_de_linhas.keys()) if banco_de_linhas else 20
